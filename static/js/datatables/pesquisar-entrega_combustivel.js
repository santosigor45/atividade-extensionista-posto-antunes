$(document).ready(() => {
    ...
    // inicializa a tabela
    let table = $('#table-entrega_combustivel').DataTable({
        lengthMenu: [[10, 25, 50, -1], [10, 25, 50, "Todos"]],
        ajax: {
            url: '/api/api_data/entrega_combustivel',
            data: (dtParms) => {
    ...
    let formatDateToBrazilian = (dateStr) => {
        if (!dateStr) return '';
    ...
    $('#table-entrega_combustivel tbody').on('click', '#btn-edit', function () {
    ...
    $('#table-entrega_combustivel tbody').on('click', '#btn-delete', function () {
        const data = table.row($(this).closest('tr')).data();
